...
🔬 IMMEDIATE NEXT STEPS FOR PUBLICATION:
...
Want to train it and see what happens? 🧠✨
"""
...
class UniversalEmbedding(nn.Module):
    ...
    def _embed_chess(self, board_tensor, positions=None):
        ...
        # Ensure squares are 0..63 when CLS is present
        offset = 1 if seq_len == 65 else 0
        squares = board_positions - offset
        ...
        # Debug guards (safe to remove later)
        assert ranks.min() >= 0 and ranks.max() <= 7, "rank index out of range"
        assert files.min() >= 0 and files.max() <= 7, "file index out of range"
        assert diagonals.min() >= 0 and diagonals.max() <= 14, "diag out of range"
        assert anti_diagonals.min() >= 0 and anti_diagonals.max() <= 14, "anti-diag out of range"
...
class AGIChessEnvironment:
    ...
    def get_state(self):
        ...
        # Always prepend CLS so seq_len == 65
        cls_token = torch.full((1,), SpecialTokens.CLS, dtype=torch.long)
        board_tensor = torch.cat([cls_token, board_tensor])
...
class AGITrainer:
    ...
    def play_game(self, use_reasoning=True, use_consciousness=True, max_moves=200):
        ...
        for move_num in range(max_moves):
            ...
            if experience['legal_move'] and 0 <= experience['action'] < ACTION_SPACE_SIZE:
                self.replay_buffer.append(experience)
            else:
                print(f"⚠️ Skipped illegal/out-of-range action: {experience['action']}")
